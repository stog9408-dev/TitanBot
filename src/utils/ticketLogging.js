...
/**
 * Log ticket feedback to the appropriate channel
 ...
 * @param {string} options.userId - User ID who gave feedback
 * @param {number} options.rating - Rating (1-5)
 * @param {string} options.comment - Optional comment
 */
export async function logTicketFeedback({ client, guildId, ticketId, userId, rating, comment }) {
  try {
    const guild = client.guilds.cache.get(guildId) || await client.guilds.fetch(guildId).catch(() => null);
    if (!guild) {
      logger.warn(`logTicketFeedback invoked without valid guild: ${guildId}`);
      return;
    }
    ...
    const channel = guild.channels.cache.get(logChannelId) || await guild.channels.fetch(logChannelId).catch(() => null);
    if (!channel) {
      logger.warn(`Ticket feedback log channel not found: ${logChannelId}`);
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Feedback for ticket <#${ticketId}>`)
      .addFields(
    ...
    logger.info(`Ticket feedback logged for ticket ${ticketId} in guild ${guildId}`);

  } catch (error) {
    logger.error('Error logging ticket feedback:', error);
  }
}

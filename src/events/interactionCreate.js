...
// Command statistics tracking functions
function trackCommandSuccess(client, commandName, startTime, userId) {
  ...
  stats.avgResponseTime = Math.round(stats.totalResponseTime / stats.uses);
  stats.users.add(userId);

  client.commandStats.set(commandName, stats);
}
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            try {
              ...
              trackCommandSuccess(client, interaction.commandName, startTime, interaction.user.id);
            } catch (executeError) {
              // Record failed execution
              trackCommandError(client, interaction.commandName, interaction.user.id);
              throw executeError;
            }
          } catch (error) {

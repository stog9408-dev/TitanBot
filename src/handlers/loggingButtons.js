...
/**
 * Handle logging menu select interactions
...
export async function handleLoggingMenuSelect(interaction) {
  try {
    ...
    // Handle the selected logging options
    for (const value of selectedValues) {
      const eventType = parseEventTypeFromButton(value);
      if (eventType) {
        await toggleEventLogging(interaction.client, interaction.guildId, eventType, true);
      }
    }
    ...
    logger.error('Error in logging menu select:', error);
    await interaction.reply({
